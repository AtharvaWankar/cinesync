...
// The Performance Resource Timing buffer only holds a few hundred entries
// (browser default is 250). A movie streamed over range requests blows
// through that in a couple of minutes, after which new entries just get
// silently dropped — which is why "Total data transferred" used to stall
// partway through playback. We raise the buffer size and, whenever it
// still fills up, bank the bytes we've counted so far into an offset and
// clear it out, so the running total never loses data.
let statsBytesOffset = 0;
if (typeof performance !== "undefined" && performance.setResourceTimingBufferSize) {
  performance.setResourceTimingBufferSize(5000);
}
if (typeof performance !== "undefined" && performance.addEventListener) {
  performance.addEventListener("resourcetimingbufferfull", () => {
    statsBytesOffset += currentVideoBytesInBuffer();
    performance.clearResourceTimings();
  });
}

function currentVideoBytesInBuffer() {
  return performance.getEntriesByType("resource")
    .filter(e => e.name.includes("/video"))
    .reduce((sum, e) => sum + (e.transferSize || 0), 0);
}

// Cumulative bytes transferred for the movie stream, session-wide —
// survives resource-timing buffer clears.
function totalVideoBytesTransferred() {
  return statsBytesOffset + currentVideoBytesInBuffer();
}
...
function updateThroughput() {
  const totalBytes = totalVideoBytesTransferred();
...
function renderStats() {
  ...
  html += row("Total data transferred", fmtBytes(totalVideoBytesTransferred()));
  html += row("Packets dropped", "N/A (not exposed by browsers)");

  // ── Video ──
  html += section("🎞️ Video");
